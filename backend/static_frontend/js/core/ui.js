...
window.PDFJIN_UI = {
    ...
    handleSelection(files, callback) {
        ...
        // ── File size enforcement at selection time ──────────────────────────
        const oversized = [];
        Array.from(files).forEach(file => {
            const sizeMB = file.size / (1024 * 1024);
            if (window.PDFJIN_TASKS) {
                const err = window.PDFJIN_TASKS.checkCanProcess(sizeMB);
                if (err && err.type === 'file_size') {
                    oversized.push({ file, err });
                }
            }
        });

        if (oversized.length > 0) {
            // Show the limit modal for the first oversized file
            if (window.PDFJIN_TASKS && typeof window.PDFJIN_TASKS.showBlockModal === 'function') {
                window.PDFJIN_TASKS.showBlockModal(oversized[0].err);
            }
            // Filter out oversized files — keep only allowed ones
            const allowed = Array.from(files).filter(f => {
                const sizeMB = f.size / (1024 * 1024);
                if (!window.PDFJIN_TASKS) return true;
                const err = window.PDFJIN_TASKS.checkCanProcess(sizeMB);
                return !(err && err.type === 'file_size');
            });
            if (!allowed.length) return; // All files were oversized — abort
            files = allowed;
        }
        // ── End file size enforcement ────────────────────────────────────────
        ...
        if (this.selectedFiles.length > 0) {
            ...
            if (dropZone && isSingle) dropZone.style.display = 'none';
        }
    },
