    ...
    async function renderAllPages() {
        ...
        for (let i = 1; i <= pdfDoc.numPages; i++) {
            ...
            textContent.items.forEach(item => {
                ...
                // Calculate real pixel dimensions from PDF metadata
                const realWidth = item.width * scale;
                const realHeight = Math.sqrt(tx[0] * tx[0] + tx[1] * tx[1]);
                
                // Adjust scale if needed
                if (item.width > 0 && span.offsetWidth > 0) {
                    const s = realWidth / span.offsetWidth;
                    if (s) span.style.transform = `scaleX(${s})`;
                }
                
                textLayer.appendChild(span);
                
                span.dataset.originalWidth = realWidth;
                span.style.minWidth = realWidth + 'px';
                span.style.minHeight = realHeight + 'px';
                span.style.display = 'inline-block';
                
                span.onfocus = () => {
                    if (activeTool !== 'select') { span.blur(); return; }
            ...
            // Removed legacy onmouseup logic. Spans are now natively contenteditable.
        }
    ...
    if (executeBtn) {
        executeBtn.onclick = async () => {
            ...
            document.querySelectorAll('.page-wrapper').forEach(wrapper => {
                ...
                // Capture Native ContentEditable Spans
                wrapper.querySelectorAll('.pdf-word.modified').forEach(span => {
                    const x = parseFloat(span.style.left) * scaleX;
                    const y = parseFloat(span.style.top) * scaleY;
                    const origW = parseFloat(span.dataset.originalWidth) * scaleX;
                    const origH = parseFloat(span.style.minHeight) * scaleY;
                    const cssSize = parseFloat(span.style.fontSize) || 12;
                    ...
                    finalEdits.push({
                        ...
                        x: x - (1 * scaleX),
                        y: y - (1 * scaleY),
                        width: origW + (2 * scaleX),
                        height: origH + (2 * scaleY),
                        color: '#ffffff'
                    });

                    // Add new text if not fully deleted
