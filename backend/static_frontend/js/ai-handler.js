...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleSelection = (file) => {
        ...
        // File size limit check at selection time
        if (window.PDFJIN_TASKS && typeof window.PDFJIN_TASKS.checkCanProcess === 'function') {
            const sizeMB = file.size / (1024 * 1024);
            const err = window.PDFJIN_TASKS.checkCanProcess(sizeMB);
            if (err && err.type === 'file_size') {
                window.PDFJIN_TASKS.showBlockModal(err);
    ...
    if (startChatBtn) {
        startChatBtn.addEventListener('click', () => {
            ...
            // Task limit check
            if (window.PDFJIN_TASKS && typeof window.PDFJIN_TASKS.checkCanProcess === 'function') {
                const sizeMB = window.AI_SELECTED_FILE.size / (1024 * 1024);
                const err = window.PDFJIN_TASKS.checkCanProcess(sizeMB);
                if (err) return window.PDFJIN_TASKS.showBlockModal(err);
            }
