...
# At 9:00 AM and 9:00 PM, it triggers the auto_blogger.py script.

Write-Host "PDFjin Auto-Blogger Scheduler Started."
Write-Host "It will automatically write and publish posts at 09:00 and 21:00."
Write-Host "Leave this window open in the background."

while ($true) {
    $now = Get-Date
    if (($now.Hour -eq 9 -or $now.Hour -eq 21) -and $now.Minute -eq 0) {
